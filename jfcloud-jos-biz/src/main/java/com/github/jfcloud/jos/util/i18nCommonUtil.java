...
public class i18nCommonUtil {

    public static void main(String[] args) {
        String path = "D:\\IdeaProjects\\jfcloud-jos-k42\\jfcloud-jos-biz\\src\\main\\resources\\Fileinfo-message.txt";
        reflectEntity(Fileinfo.class,path);
    }

    public static void reflectEntity(Class entity,String path){
    ...
    public static void writeFilePage(List<String> classInfo,
        ...
        try {
            ...
            String pre = "page.placeholder." + clazzName + ".";
            writeWithoutBlank(fos,superInfo,pre);
            writeWithoutBlank(fos,selfInfo,pre);

            fos.write('\n');

            pre = "page.error." + clazzName + ".";
            writeWithoutBlank(fos,superInfo,pre);
            writeWithoutBlank(fos,selfInfo,pre);
    ...
    public static void writeFileCode(List<String> classInfo,
        ...
        try {
            ...
            String pre = "code.model.";
            String clazzName = classInfo.get(0);
            clazzName = (char)(clazzName.charAt(0)+32) + clazzName.substring(1); // 将首字母大写变小写

            String first = pre + clazzName + "=" + classInfo.get(1) + "\n";
            fos.write(first.getBytes());

            pre = pre + clazzName + ".";
            writeWithBlank(fos,superInfo,pre);
            writeWithBlank(fos,selfInfo,pre);
